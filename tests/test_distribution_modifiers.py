...
def test_e():
    sides = 8
    dist = distribution(f"1d{sides}e8")
    base_chance = 1 / sides
    for roll in dist.keys():
        depth = (roll - 1) // sides + 1
        expected = base_chance**depth
        actual = dist.get(roll)
        assert expected == approx(actual)


def test_e_2():
    dist = distribution("1d8e8")
...
def test_e_gt():
    ...
    sides = 8
    threshold = 4
    dist = distribution(f"1d{sides}e>{threshold}")
    base_odds = 1 / sides
...
def test_e_lt():
    ...
    for i in range(1, threshold - 1):
        ...
        assert actual == approx(
            0
        ), f"Result {i} should be impossible (0%) because it triggers an explosion, but got {actual}."

    actual_threshold_odds = dist.get(threshold)
    msg = (
...
def test_mi_out_of_bounds():
    dist = distribution("1d20mi21")

    assert dist.get(21) == approx(1.0)
    assert dist.mean() == approx(21.0)
...
def test_valid_operations():
    ...
    distribution("4d6kh3rol2")


def test_1d20adv():
    dist = distribution("1d20adv")
...
def test_4d6dis3():
    dist = distribution("4d6dis3")

    # Verified using anydice.com
    # output [lowest of 4d6 and [lowest of 4d6 and 4d6]]
    values = [
        (4, 0.0023130291516662),
        (5, 0.0092164492830578),
        (6, 0.022792416669078),
        (7, 0.044528319803492),
        (8, 0.07459239369719),
        (9, 0.11078653662871),
        ...
        (12, 0.14714740638175),
        (13, 0.12103737504787),
        (14, 0.084848164626047),
        (15, 0.049521028454358),
        (16, 0.024128216281152),
        (17, 0.0096698616356284),
        (18, 0.0030969747817726),
        (19, 0.00076138802331773),
        (20, 0.00013787552160658),
        (21, 0.000018146049490912),
        (22, 0.0000014930293884928),
        (23, 0.000000056964813591725),
        (24, 4.5939365799778e-10),
    ]

    assert_distribution(dist, values, eps=1e-6)


def test_4d6mi3ma5adv2kh2():
    dist = distribution("4d6mi3ma5adv")

    # Verified using anydice.com
    # output [highest of 4d{3,3,3,4,5,5} and 4d{3,3,3,4,5,5}]
    values = [
        (12, 0.00390625),
        (13, 0.017361111111111),
        (14, 0.10416666666667),
        (15, 0.15556412894376),
        (16, 0.28614099889499),
        (17, 0.19040542600213),
        (18, 0.16975308641975),
        (19, 0.048163389727176),
        (20, 0.024538942234415),
    ]
